function [net, stats] = cnn_train_revised(net, imdb, getBatch, varargin)
...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    ...
    case 'square'
      opts.errorFunction = @error_square ;
...
% -------------------------------------------------------------------------
function err = error_square(params, labels, res)
% -------------------------------------------------------------------------
predictions = gather(res(end-1).x) ;
error = bsxfun(@(x,y) (x-y).^2, predictions, labels);
err = sum(error(:)) ;
